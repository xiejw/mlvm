...
func TestTensorBinaryOps(t *testing.T) {
	//--- ir
	b := ir.NewBuilder()
	f, err := b.NewFn("main")
	assertNoErr(t, err)
	...
	s := f.ShapeLiteral([]int{1, 2}).GetResult()
	a := f.ArrayLiteral([]float32{2, 3}).GetResult()
	t1 := f.TensorNew(s, a).GetResult()
	t2 := f.TensorAdd(t1, t1).GetResult()
	t3 := f.TensorMinus(t2, t2).GetResult()
	t4 := f.TensorMul(t3, t3).GetResult()
	t5 := f.TensorDiv(t4, t4).GetResult()
	f.SetOutputAndDone(t5)
